...
from vq.vocos_encoder import VocosFormerCodecEncoderWrapper
...
class SdpCodecLightningModule(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        use_vocosformer = bool(getattr(enccfg, 'use_vocosformer', False))
        freeze_vocosformer = bool(getattr(enccfg, 'freeze_vocosformer_encoder', False))
        ...
        elif use_vocosformer and not freeze_vocosformer:
            self._freeze_schedule_enabled = False
            print(colored("freeze_vocosformer_encoder=False -> disabling unfreeze_encoder_step; vocosformer CodecEnc stays UNFROZEN.", "yellow"))
        else:
        ...
        if self.unfreeze_encoder_step is not None and use_vocosformer and freeze_vocosformer:
            print(colored("Both train.unfreeze_encoder_step and codec_encoder.freeze_vocosformer_encoder are set. Step-based schedule will override encoder freeze after unfreeze step.", "yellow"))
    ...
    def on_fit_start(self):
        ...
        if self.use_val_wavlm and self.wavlm_model is None:
            self._load_wavlm_model(device=self.device, reason="fit_start_preload")
    ...
    def construct_model(self):
        ...
        if use_codec_structure:
            ...
            if f0_encoder_style in {'hoyeol', 'hoyeol_style'}:
                encoder_total_layers = int(f0cfg.get('encoder_num_layers', 4))
                encoder_level2_num_layers = int(
                    f0cfg.get('encoder_level2_num_layers', encoder_total_layers // 2)
                )
                encoder_level1_num_layers = int(
                    f0cfg.get('encoder_level1_num_layers', encoder_total_layers - encoder_level2_num_layers)
                )
                f0_encoder_downsample_factor = int(np.prod(f0_encoder_cfg.up_ratios)) if f0_encoder_cfg.up_ratios else 1
                f0_encoder = HoyeolStyleF0CodecEncoder(
                    input_channels=input_channels,
                    out_channels=encoder_out_channels,
                    dim=int(f0cfg.get('encoder_dim', encoder_out_channels)),
                    intermediate_dim=int(f0cfg.get('encoder_intermediate_dim', encoder_out_channels * 3)),
                    downsample_factor=f0_encoder_downsample_factor,
                    level2_num_layers=encoder_level2_num_layers,
                    level1_num_layers=encoder_level1_num_layers,
                    num_heads=int(f0cfg.get('encoder_num_heads', 4)),
                    attn_window_size=tuple(f0cfg.get('encoder_attn_window_size', [64, 64])),
                    attention_impl=str(f0cfg.get('encoder_attention_impl', 'flash_attn')).lower(),
                    use_rope=bool(f0cfg.get('encoder_use_rope', True)),
                    rope_base=float(f0cfg.get('encoder_rope_base', 10000.0)),
                    max_position_embeddings=int(f0cfg.get('encoder_max_position_embeddings', 2048)),
                    qk_norm=bool(f0cfg.get('encoder_qk_norm', True)),
                    norm_type=str(f0cfg.get('encoder_norm_type', 'rmsnorm')).lower(),
                    norm_eps=float(f0cfg.get('encoder_norm_eps', 1e-2)),
                    dropout=float(f0cfg.get('encoder_dropout', 0.0)),
                    ffn_type=str(f0cfg.get('encoder_ffn_type', 'swiglu')).lower(),
                    ffn_mult=float(f0cfg.get('encoder_ffn_mult', 4.0)),
                    layerscale_gamma_init=float(f0cfg.get('encoder_layerscale_gamma_init', 1.0)),
                )
            else:
            ...
            if f0_decoder_style in {'hoyeol', 'hoyeol_style'}:
                decoder_total_layers = int(f0cfg.get('decoder_num_layers', 4))
                decoder_level2_num_layers = int(
                    f0cfg.get('decoder_level2_num_layers', decoder_total_layers // 2)
                )
                decoder_level1_num_layers = int(
                    f0cfg.get('decoder_level1_num_layers', decoder_total_layers - decoder_level2_num_layers)
                )
                f0_decoder_upsample_factor = int(np.prod(f0_decoder_cfg.up_ratios)) if f0_decoder_cfg.up_ratios else 1
                f0_decoder = HoyeolStyleF0CodecDecoder(
                    in_channels=decoder_in_channels,
                    output_channels=output_channels,
                    dim=int(f0cfg.get('decoder_dim', decoder_in_channels)),
                    intermediate_dim=int(f0cfg.get('decoder_intermediate_dim', decoder_in_channels * 3)),
                    ...
                    num_heads=int(f0cfg.get('decoder_num_heads', 8)),
                    attn_window_size=tuple(f0cfg.get('decoder_attn_window_size', [64, 64])),
                    attention_impl=str(f0cfg.get('decoder_attention_impl', 'flash_attn')).lower(),
                    use_rope=bool(f0cfg.get('decoder_use_rope', True)),
                    rope_base=float(f0cfg.get('decoder_rope_base', 10000.0)),
                    max_position_embeddings=int(f0cfg.get('decoder_max_position_embeddings', 2048)),
                    qk_norm=bool(f0cfg.get('decoder_qk_norm', True)),
                    norm_type=str(f0cfg.get('decoder_norm_type', 'rmsnorm')).lower(),
                    norm_eps=float(f0cfg.get('decoder_norm_eps', 1e-2)),
                    dropout=float(f0cfg.get('decoder_dropout', 0.0)),
                    ffn_type=str(f0cfg.get('decoder_ffn_type', 'swiglu')).lower(),
                    ffn_mult=float(f0cfg.get('decoder_ffn_mult', 4.0)),
                    layerscale_gamma_init=float(f0cfg.get('decoder_layerscale_gamma_init', 1.0)),
                    stage_output_repeat=int(f0cfg.get('decoder_stage_output_repeat', 2)),
                )
            else:
        ...
        use_s3tokenizer = bool(getattr(enccfg, 'use_s3tokenizer', False))
        use_vocosformer = bool(getattr(enccfg, 'use_vocosformer', False))
        if use_vocosformer and not (use_w2v2 or use_hubert):
            raise ValueError("codec_encoder.use_vocosformer=True currently requires use_w2v2=True or use_hubert=True.")
        ...
        if use_vqw2v:
            _set_s3prl_download_dir(configure_s3prl_cache_root())
            ...
            encoder = S3TokenizerEncoder(
                ...
                sampling_rate=sampling_rate,
                use_continuous=True,
                force_eval=freeze_s3tokenizer,
            )
            if freeze_s3tokenizer:
        ...
        if use_w2v2:
            ...
            if use_vocosformer:
                vocos_cfg = enccfg.get('vocos', {})
                print(colored(f"Using Wav2Vec2 VocosFormer Codec Encoder Wrapper (up_ratios={enccfg.get('up_ratios')})", "red"))
                encoder = VocosFormerCodecEncoderWrapper(
                    encoder=encoder,
                    input_channels=int(w2v2_ngf),
                    up_ratios=enccfg.get('up_ratios'),
                    dilations=enccfg.get('dilations', [1, 3, 9]),
                    out_channels=enccfg.out_channels,
                    dim=vocos_cfg.get('dim', 512),
                    intermediate_dim=vocos_cfg.get('intermediate_dim', 1536),
                    num_layers=vocos_cfg.get('num_layers', 2),
                    num_heads=vocos_cfg.get('num_heads', 8),
                    window_size=vocos_cfg.get('window_size', None),
                    attn_window_size=vocos_cfg.get('attn_window_size', None),
                    attention_impl=vocos_cfg.get('attention_impl', 'flash_attn'),
                    use_rope=vocos_cfg.get('use_rope', True),
                    rope_base=vocos_cfg.get('rope_base', 10000.0),
                    max_position_embeddings=vocos_cfg.get('max_position_embeddings', 2048),
                    qk_norm=vocos_cfg.get('qk_norm', True),
                    norm_type=vocos_cfg.get('norm_type', 'rmsnorm'),
                    norm_eps=vocos_cfg.get('norm_eps', 0.01),
                    dropout=vocos_cfg.get('dropout', 0.0),
                    ffn_type=vocos_cfg.get('ffn_type', 'swiglu'),
                    ffn_mult=vocos_cfg.get('ffn_mult', 4.0),
                    layerscale_gamma_init=vocos_cfg.get('layerscale_gamma_init', 1.0),
                    activation_type=enccfg.get('activation_type', 'SnakeBeta'),
                    leaky_relu_params=enccfg.get('leaky_relu_params', None),
                    encoder_force_fp32=enccfg.get('frozen_upstream_force_fp32', False),
                    snake_lite_taylor_degree=enccfg.get('snake_lite_taylor_degree', 8),
                    downsample_mode=vocos_cfg.get('downsample_mode', 'conv'),
                    pre_num_layers=vocos_cfg.get('pre_num_layers', 0),
                    post_num_layers=vocos_cfg.get('post_num_layers', None),
                    downsample_dim=vocos_cfg.get('downsample_dim', None),
                    downsample_activation=vocos_cfg.get('downsample_activation', 'silu'),
                )
            else:
        ...
        if use_hubert:
            ...
            if use_vocosformer:
                vocos_cfg = enccfg.get('vocos', {})
                print(colored(f"Using HuBERT VocosFormer Codec Encoder Wrapper (up_ratios={enccfg.get('up_ratios')})", "red"))
                encoder = VocosFormerCodecEncoderWrapper(
                    encoder=encoder,
                    input_channels=int(hubert_ngf),
                    up_ratios=enccfg.get('up_ratios'),
                    dilations=enccfg.get('dilations', [1, 3, 9]),
                    out_channels=enccfg.out_channels,
                    dim=vocos_cfg.get('dim', 512),
        ...
        decoder_type_name = str(deccfg.get('decoder_type', 'default')).lower().replace("_", "").replace("-", "")
        decoder_type_aliases = {
            "vocosformerdecoder": "vocosformer",
            "dtmdecoder": "dtm",
            "apnformerdecoder": "apnformer",
        }
        decoder_type_name = decoder_type_aliases.get(decoder_type_name, str(deccfg.get('decoder_type', 'default')).lower())
        if decoder_type_name in ['rndvoc', 'vocos', 'vocosformer', 'dtm', 'apnformer'] and decoder_f0_cond_cfg.use_concat:
            stage_count = int(
                deccfg.get(
                    'rndvoc_null_nstage',
                    decoder_vocos_cfg.num_layers,
                )
            ) if decoder_type_name == 'rndvoc' else int(decoder_vocos_cfg.num_layers)
        ...
        decoder = CodecDecoder(
                    ...
                    fsq_embedding_codebook_loss_weight=deccfg.get('fsq_embedding_codebook_loss_weight', 1.0),
                    codebook_size=deccfg.codebook_size,
                    codebook_dim=deccfg.get('codebook_dim', 8),
                    fsq_levels=deccfg.get('fsq_levels', None),
                    fsq_num_quantizers=deccfg.get('fsq_num_quantizers', None),
                    ...
                    vocos_level2_num_layers=decoder_vocos_cfg.level2_num_layers,
                    vocos_level1_num_layers=decoder_vocos_cfg.level1_num_layers,
                    vocos_use_frame_upsample=decoder_vocos_cfg.use_frame_upsample,
                    vocos_frame_upsample_factor=decoder_vocos_cfg.frame_upsample_factor,
        ...
        for extra_spec_type in self._extra_spectral_discriminator_types():
            ...
            elif extra_spec_type == 'bigvgan_mrd':
                spec_cfg = self.cfg.model.get('spec_discriminator', {})
                mrdcfg = spec_cfg.get('bigvgan_mrd', {})
                extra_spec_discs[extra_key] = BigVGANMultiResolutionDiscriminator(
                    resolutions=tuple(tuple(int(v) for v in resolution) for resolution in mrdcfg.get(
                        'resolutions',
                        ((1024, 120, 600), (2048, 240, 1200), (512, 50, 240)),
                    )),
            ...
            elif extra_spec_type == 'vocos_mrd':
                spec_cfg = self.cfg.model.get('spec_discriminator', {})
                mrdcfg = spec_cfg.get('vocos_mrd', {})
                extra_spec_discs[extra_key] = VocosMultiResolutionDiscriminator(
                    fft_sizes=tuple(int(v) for v in mrdcfg.get('fft_sizes', (2048, 1024, 512))),
                    channels=int(mrdcfg.get('channels', 32)),
                    hop_factor=float(mrdcfg.get('hop_factor', 0.25)),
                    bands=tuple(tuple(float(v) for v in band) for band in mrdcfg.get(
            ...
            elif extra_spec_type == 'apnet2_mrd':
                spec_cfg = self.cfg.model.get('spec_discriminator', {})
                mrdcfg = spec_cfg.get('apnet2_mrd', {})
                extra_spec_discs[extra_key] = APNet2MultiResolutionDiscriminator(
                    resolutions=tuple(tuple(int(v) for v in resolution) for resolution in mrdcfg.get(
                        'resolutions',
                        ((1024, 256, 1024), (2048, 512, 2048), (512, 128, 512)),
                    )),
    ...
    def _iter_discriminator_modules(self):
        for name in (
            ...
            'extra_bigvgan_mrd_discriminator',
            'extra_vocos_mrd_discriminator',
            'extra_apnet2_mrd_discriminator',
            'cqt_discriminator',
        ):
    ...
    def _extra_spectral_discriminator_types(self) -> list[str]:
        ...
        ordered = ("mstft", "bigvgan_mrd", "vocos_mrd", "apnet2_mrd")
        ...
        if self.cfg.train.get('use_torch_compile', False):
            ...
            if hasattr(self, 'model'):
                ...
                disc_labels = {
                    ...
                    'extra_bigvgan_mrd_discriminator': self._spectral_discriminator_label('bigvgan_mrd'),
                    'extra_vocos_mrd_discriminator': self._spectral_discriminator_label('vocos_mrd'),
                    'extra_apnet2_mrd_discriminator': self._spectral_discriminator_label('apnet2_mrd'),
    ...
    def forward(self, batch):
        ...
        if use_vqw2v:
            ...
            with torch.set_grad_enabled(not enc_no_grad):
                vq_emb = self.model['CodecEnc'](wav)  # [B, C, T]
        elif use_w2v2:
            ...
            raise NotImplementedError("Forward is only available when using a supported codec encoder (vq-wav2vec, w2v2, hubert, w2vbert2, s3tokenizer, or vocosformer).")
        if parallel_branch_streams:
    ...
    def tokenizer(self, batch):
        ...
        with torch.no_grad():
            ...
            if use_vqw2v:
                ...
                vq_emb = self.model['CodecEnc'](batch['wav'])  # [B, C, T]
            elif use_w2v2:
                enc_cfg = self.cfg.model.codec_encoder
                legacy = bool(getattr(enc_cfg, "w2v2_legacy_input", False))
                if legacy:
                ...
                raise NotImplementedError("Tokenizer is only available when using a supported codec encoder (vq-wav2vec, w2v2, hubert, w2vbert2, s3tokenizer, or vocosformer).")
    ...
    def compute_gen_loss(self, batch, output, cached_real=None, cached_spec_real=None, cached_input_crops=None):
        ...
        if use_feat_match:
            ...
            extra_mrd_fm_loss = torch.tensor(0.0, device=self.device)
            extra_mrd_fm_seen = False
            for name, value in fm_loss_by_name.items():
                extra_spec_type = self._extra_spectral_type_from_key(name)
                if extra_spec_type is None:
                    continue
                loss_dict[f'{extra_spec_type}_fm_loss'] = value
                if extra_spec_type in {'bigvgan_mrd', 'vocos_mrd', 'apnet2_mrd'}:
                    extra_mrd_fm_loss = extra_mrd_fm_loss + value
                    extra_mrd_fm_seen = True
            if extra_mrd_fm_seen:
                loss_dict['mrd_fm_loss'] = extra_mrd_fm_loss
        ...
        if use_fcpe and f0_fcpe_latent is not None and gt_f0 is not None:
            ...
            loss_dict['fcpe_active_ratio'] = fcpe_active_ratio
            ...
            loss_dict['fcpe_active_ratio'] = torch.tensor(0.0, device=self.device)
            ...
            try:
                with torch_hub_cache_lock(torch_cache_dir, name="speechmos"):
                    ...
                    if private_repo is None:
                        try:
                            bootstrap_predictor = torch.hub.load(
                                "tarepan/SpeechMOS:v1.2.0",
                                "utmos22_strong",
                                trust_repo=True,
                                force_reload=True,
                            )
                            del bootstrap_predictor
    ...
    def _wavlm_model_device(self):
        if self.wavlm_model is None:
            return None
        try:
            return next(self.wavlm_model.parameters()).device
        except StopIteration:
            return None

    def _move_wavlm_model(self, device: torch.device, reason: str) -> None:
        if self.wavlm_model is None:
            return
        current_device = self._wavlm_model_device()
        if current_device == device:
            return
        print(colored(f"Moving WavLM_large model to {device} ({reason})", "yellow"))
        self.wavlm_model = self.wavlm_model.to(device)
        self.wavlm_model.eval()

    def _load_wavlm_model(self, device: Optional[torch.device] = None, reason: str = "validation") -> None:
        if device is None:
            device = self.device
        device = torch.device(device)
        print(colored(f"Loading WavLM_large model ({reason}, device={device})...", "yellow"))
        _set_s3prl_download_dir(configure_s3prl_cache_root())
        ...
        self.wavlm_model = ECAPA_TDNN_SMALL(feat_dim=1024, feat_type='wavlm_large', config_path=None).to(device)
        self.wavlm_model.eval()
        checkpoint_path = resolve_wavlm_sv_checkpoint_path(
            explicit_path=getattr(self.cfg.train, "val_wavlm_checkpoint_path", None),
        ...
        print(colored(f"✓ WavLM loaded with checkpoint ({checkpoint_path}, {reason}, device={device})", "green"))
    ...
    def on_validation_epoch_start(self):
        ...
        elif self.use_val_wavlm:
            self._move_wavlm_model(self.device, reason="validation")
    ...
    def on_validation_epoch_end(self):
        ...
        if self.wavlm_model is not None:
            if self._val_metric_preload_enabled:
                print(colored("✓ WavLM model kept on validation device", "cyan"))
            else:
        ...
        if self.val_step_plot_outputs and is_main:
            ...
            for i, sample in enumerate(samples[:rows * cols]):
                ...
                duration_code_ids = sample.get('duration_code_ids')
    ...
    def _append_wavlm_scores(
        ...
        if self.wavlm_model is None or ref_audio_16k.numel() == 0 or hyp_audio_16k.numel() == 0:
            return
        num_items = min(ref_audio_16k.size(0), hyp_audio_16k.size(0))
        if num_items <= 0:
            return
        try:
            chunk_size = int(self.cfg.train.get("val_wavlm_metric_batch_size", 8))
        except Exception:
            chunk_size = 8
        chunk_size = max(1, min(chunk_size, num_items))

        start = 0
        while start < num_items:
            cur_chunk = min(chunk_size, num_items - start)
            while cur_chunk >= 1:
                end = start + cur_chunk
                try:
                    ref_emb = self.wavlm_model(ref_audio_16k[start:end])
                    hyp_emb = self.wavlm_model(hyp_audio_16k[start:end])
                    cosine_sim = torch.nn.functional.cosine_similarity(ref_emb, hyp_emb, dim=-1)
                    score_store.extend(cosine_sim.detach().cpu().tolist())
                    start = end
                    break
                except Exception as e:
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                    if cur_chunk == 1:
                        print(colored(f"WavLM error ({tag}) sample={start}: {e}", "red"))
                        start += 1
                        break
                    next_chunk = max(1, cur_chunk // 2)
                    print(colored(
                        f"WavLM batch error ({tag}, batch={cur_chunk}), retrying with batch={next_chunk}: {e}",
                        "red",
                    ))
    ...
    def _gather_loss_dict(self, disc_losses, gen_losses, mi_losses=None):
        ...
        losses = {
            ...
            'mrd_fm_loss': gen_losses.get('mrd_fm_loss', z()),
            'bigvgan_mrd_fm_loss': gen_losses.get('bigvgan_mrd_fm_loss', z()),
            'vocos_mrd_fm_loss': gen_losses.get('vocos_mrd_fm_loss', z()),
            'apnet2_mrd_fm_loss': gen_losses.get('apnet2_mrd_fm_loss', z()),
            ...
            'fcpe_active_ratio': gen_losses.get('fcpe_active_ratio', z()),
        }
        if apn_cfg is not None:
            losses.update({
        ...
        _scale('mrd_fm_loss', 'lambda_feat_match_loss')
        _scale('bigvgan_mrd_fm_loss', 'lambda_feat_match_loss')
        _scale('vocos_mrd_fm_loss', 'lambda_feat_match_loss')
        _scale('apnet2_mrd_fm_loss', 'lambda_feat_match_loss')
